    ...
    boolean uni_checkWin(char dot) {
        ...
        for(int i = 0; i < SIZE; i++) {
            for(int j = 0; j < SIZE; j++) {
                if (map[i][j]==dot) {
                    CUR_SERIES=1;
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл вниз
                        if ((i+k) >= SIZE) break;         // выход вниз
                        if(map[i+k][j] == dot)  CUR_SERIES++;
                        else break;
                    }
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл вверх
                        if ((i-k) < 0) break;             // выход вверх
                        if(map[i-k][j] == dot)  CUR_SERIES++;
                        else break;
                    }
                    if (CUR_SERIES>=WIN_SERIES) return true;
                    CUR_SERIES=1;
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл вправо
                        if ((j+k) >= SIZE) break;         // выход вправо
                        if(map[i][j+k] == dot)  CUR_SERIES++;
                        else break;
                    }
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл влево
                        if ((j-k) < 0) break;         // выход влево
                        if(map[i][j-k] == dot)  CUR_SERIES++;
                        else break;
                    }
                    if (CUR_SERIES>=WIN_SERIES) return true;
                    CUR_SERIES=1;
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл диагональ \ вниз
                        if ((i+k) >= SIZE || (j+k) >= SIZE) break;     // выход вниз
                        if(map[i+k][j+k] == dot)  CUR_SERIES++;
                        else break;
                    }
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл диагональ \ вверх
                        if ((i-k) < 0 || (j-k) < 0 ) break;               // выход вверх
                        if(map[i-k][j-k] == dot)  CUR_SERIES++;
                        else break;
                    }
                    if (CUR_SERIES>=WIN_SERIES) return true;
                    CUR_SERIES=1;
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл диагональ / вниз
                        if ((i+k) >= SIZE || (j-k) <0 ) break;     // выход вниз
                        if(map[i+k][j-k] == dot)  CUR_SERIES++;
                        else break;
                    }
                    for (int k = 1; k <= WIN_SERIES; k++){  // цикл диагональ / вверх
                        if ((i-k) < 0 || (j+k) >= SIZE ) break;               // выход вверх
                        if(map[i-k][j+k] == dot)  CUR_SERIES++;
                        else break;
                    }
                    if (CUR_SERIES>=WIN_SERIES) return true;
